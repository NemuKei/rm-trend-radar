...
RSS の定期取得を追加しても、掲載判断は自動化しない。定期取得で追加された記事は未確認記事として扱い、`public_candidate` は自動で `1` にしない。副業リポ側 LP に掲載するかどうかは、利用者が原文または必要な周辺情報を確認し、公開してよい短い紹介になっていることを確認してから保存する。
...
副業リポ側の初期実装では、上記のデータを静的配列として LP のコード内または LP が既に使っているローカルデータファイルに置いてよい。`rm-trend-radar` からの自動同期、API 化、定期更新、GitHub Actions 連携は初期実装の対象外とする。`rm-trend-radar` 側で RSS の定期取得を追加する場合でも、この制約は変えない。
