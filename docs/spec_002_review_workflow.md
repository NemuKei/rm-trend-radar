...
- 選択した記事について、公開候補フラグ、要約、示唆、メモを編集できる。
...
初期実装では、`public_candidate` を保存し、画面で絞り込めるところまでを扱う。複業リポ側 LP へ直接書き込む処理、X 投稿文の作成、X への投稿または予約投稿、掲載ページ構成、公開前チェックの詳細は後続タスクで仕様化する。
