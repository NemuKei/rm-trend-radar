...
- 公開対象の制約なしで、定期取得された未確認記事を自動的に公開候補へ確定すること
- 記事本文全文、長文要約、内部項目を含めた副業リポ側 LP への自動反映
- 検証なしで副業リポ側 LP のファイルを直接更新すること
- X への投稿または予約投稿
...
- 公開前チェック: `summary_ja` が原文記事の代替になるほど長くないこと、専門家コメントや記事構成を詳細に再現していないこと、原文 URL が表示されていることを確認する。自動更新の場合も、この確認は機械的な検証条件として実行する。
...
### Japanese Title Index Growth Rule

副業リポ側 LP の日本語タイトル一覧は、確認済みの公開候補記事が増える前提で表示量を制御する。

- カテゴリごとの初期表示件数は、最大 8 件を標準とする。
- カテゴリ内の記事数が初期表示件数を超える場合、超過分のタイトルは初期表示では隠す。
- 隠したタイトルは、カテゴリ内の追加表示操作、折りたたみ解除、または詳細記事一覧へのリンクで確認できるようにする。
- 初期表示では、カテゴリごとの総件数を表示する。
- 並び順は、`published_date` の新しい順を維持する。
- 自動更新で記事数が増えても、日本語タイトル一覧がページ全体の主要領域を過度に占有しないようにする。
